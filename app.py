...
Version : 3.0 — Tous correctifs intégrés

Corrections :
  - Upload CSV corrigé (lecture en bytes)
  - st.rerun() après upload
  - Vérification df_raw avant affichage filtres
  - Authentification sans hash (comparaison directe)
  - Variables déclarées avant les blocs with
"""
...
.result-box .rlabel { color: #a8b4c8; font-size: 0.9rem; margin-top: 0.4rem; }
.sec-badge { display:inline-flex; align-items:center; gap:0.4rem; background:rgba(46,204,113,0.15); border:1px solid rgba(46,204,113,0.4); color:#2ecc71; border-radius:20px; padding:0.2rem 0.75rem; font-size:0.72rem; font-weight:600; }
...
def check_credentials(username: str, password: str) -> bool:
    """Vérifie les credentials — comparaison directe (secrets.toml)."""
    try:
        valid_user = st.secrets["auth"]["username"]
        valid_pass = st.secrets["auth"]["password"]
        return username == valid_user and password == valid_pass
    except Exception:
        # Fallback local sans secrets.toml
...
def login_page():
    ...
    <div style="max-width:420px;margin:4rem auto;padding:2.5rem;
        ...
        <div style="font-family:'DM Serif Display',serif;font-size:1.8rem;color:#f5e6c8;text-align:center;margin-bottom:0.3rem;">
            🏠 ImmoPredict
        </div>
        <div style="color:#a8b4c8;text-align:center;font-size:0.88rem;margin-bottom:1.5rem;">
            Connexion requise
        </div>
    </div>
    """, unsafe_allow_html=True)
        ...
        with st.form("login_form"):
            ...
            if submitted:
                if not username or not password:
                    ...
                    log_event("LOGIN_SUCCESS", f"user={username.strip()}")
                    st.rerun()
                else:
                    st.error("Identifiant ou mot de passe incorrect.", icon="🔒")
                    log_event("LOGIN_FAIL", f"user={username.strip()}")

        st.markdown(
            '<div style="text-align:center;font-size:0.78rem;color:#8899aa;margin-top:0.8rem;">'
            '🔒 Connexion sécurisée — HTTPS · Protection XSRF</div>',
            unsafe_allow_html=True,
        )
...
def load_data(file_bytes: bytes) -> pd.DataFrame:
    """Charge un CSV depuis des bytes (compatible Streamlit Cloud)."""
...
with st.sidebar:
    ...
    <div style="padding:1rem 0.5rem 0.5rem;">
        ...
        <div style="font-size:0.74rem;color:#6688aa;font-style:italic;">Étudiant · M2 Data & Intelligence Artificielle</div>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("---")
    st.markdown(
        f'<div class="sec-badge">🔒 {st.session_state.username}</div>',
        unsafe_allow_html=True,
    )
...
if page == "Page 1":
    ...
    with col_up:
        uploaded = st.file_uploader(
            "📂 Charger train.csv",
            type="csv",
            help="CSV Ames Housing — max 10 Mo.",
        )
    with col_info:
        st.markdown("""
        <div class="info-box">
        ...
        CSV Ames Housing :<br>
        SalePrice, GrLivArea, Neighborhood…<br><br>
        <b>Limite :</b> Max 10 Mo
        </div>""", unsafe_allow_html=True)
    ...
    if uploaded is not None:
        ...
        if len(file_bytes) == 0:
            ...
            st.rerun()  # ← recharge la page pour afficher filtres + visu
    ...
    if st.session_state.df_raw is None:
        st.info("👆 Chargez votre fichier **train.csv** ci-dessus pour afficher les filtres et visualisations.", icon="ℹ️")
        st.stop()
    ...
    with tab1:
        ...
        with ca:
            col_var = st.selectbox("Variable",
                                   ["SalePrice","GrLivArea","SurfaceTotale","AgeLogement","OverallQual"],
                                   help="Variable à visualiser.")
        with cb:
            col_log = st.checkbox("Échelle log", value=(col_var == "SalePrice"),
                                  help="log1p pour les variables asymétriques.")
        ...
        info(f"📌 Moy: <b>{df[col_var].mean():,.1f}</b> · Méd: <b>{df[col_var].median():,.1f}</b> · "
    ...
    with tab2:
        ...
        cats = (df[col_color].astype("category").cat.codes
                if df[col_color].dtype == "object" or df[col_color].nunique() <= 15
                else df[col_color])
        sc = ax.scatter(df["GrLivArea"], df["SalePrice"], c=cats, cmap="viridis",
        ...
        ax.text(0.03, 0.95, f"r = {r:.3f}", transform=ax.transAxes, fontsize=11,
                fontweight="600", color=DARK_BLUE, va="top",
                bbox=dict(boxstyle="round,pad=0.3", facecolor="#f0f7ff", edgecolor="#c0d0e0"))
    ...
    with tab3:
        ...
        bp = ax.boxplot(
            [df[df["Neighborhood"] == nb][box_var].dropna().values for nb in order],
    ...
    with tab4:
        ...
        sel_corr = st.multiselect(
            "Variables",
            options=[c for c in df.select_dtypes("number").columns if c != "Id"],
            default=num_def,
            help="Variables pour la matrice de Pearson.",
        )
        if len(sel_corr) < 2:
    ...
    with tab5:
        ...
        prio    = [c for c in ["Id","Neighborhood","MSZoning","SalePrice","GrLivArea",
                                "SurfaceTotale","OverallQual","AgeLogement","NbSallesDeBain"]
                   if c in df_disp.columns]
        df_disp = df_disp[prio + [c for c in df_disp.columns if c not in prio]]

        st.dataframe(
            df_disp.style
                   .background_gradient(cmap="Blues", subset=["SalePrice"])
        ...
        dl1.download_button("⬇️ Données filtrées (CSV)", buf.getvalue().encode(),
                            f"filtered_{len(df)}.csv", "text/csv",
                            help="Télécharge les données filtrées.")
        buf2 = io.StringIO(); df.describe().to_csv(buf2)
        dl2.download_button("📈 Stats descriptives (CSV)", buf2.getvalue().encode(),
                            "stats.csv", "text/csv")
        buf3 = io.StringIO()
        df.groupby("Neighborhood")["SalePrice"].agg(["count","mean","median"]).round(0).to_csv(buf3)
        dl3.download_button("📍 Résumé quartiers (CSV)", buf3.getvalue().encode(),
                            "quartiers.csv", "text/csv")
...
# ╚══════════════════════════════════════════════════════════════════╝
    ...
    if st.session_state.df_raw is None:
        st.warning("⚠️ Chargez d'abord vos données en **Page 1**.", icon="⚠️")
        st.stop()
    ...
    with cfg2:
        test_size  = st.slider("📊 Taille test (%)", 10, 40, 20, step=5,
                               help="% des données réservé au test.")
    with cfg3:
        target_log = st.checkbox("🔁 Log-transformer la cible", value=True,
                                 help="Applique log1p(SalePrice).")
    ...
    with st.expander("⚙️ Hyperparamètres avancés"):
        ...
        with ha2: max_depth    = st.slider("max_depth (RF)", 2, 20, 0, help="0 = illimité")
        with ha3: lr_gb        = st.slider("learning_rate (GB)", 0.01, 0.3, 0.05, step=0.01)

    @st.cache_data(show_spinner="Préparation des données…")
    def prepare(df_raw_bytes, test_sz, log_target):
        df = impute(pd.read_csv(io.BytesIO(df_raw_bytes)))
    ...
    # Sérialiser df_raw en bytes pour le cache
    buf_cache = io.BytesIO()
    df_raw.to_csv(buf_cache, index=False)
    df_raw_bytes = buf_cache.getvalue()

    X_train, X_test, y_train, y_test, feat_cols, les = prepare(df_raw_bytes, test_size, target_log)
    if X_train is None:
        st.error("Colonne SalePrice introuvable.", icon="🚨")
        st.stop()
    ...
    if st.button("🚀 Entraîner le modèle", use_container_width=False):
        ...
        model_defs = {
            ...
            "Random Forest":       RandomForestRegressor(n_estimators=n_estimators, max_depth=md_val,
                                                          min_samples_split=5, random_state=42, n_jobs=-1),
            "Gradient Boosting":   GradientBoostingRegressor(n_estimators=n_estimators, learning_rate=lr_gb,
                                                              max_depth=4, subsample=0.8, random_state=42),
        }
        to_train = list(model_defs.keys()) if model_choice == "Comparer les 3" else [model_choice]
        results  = {}
        prog = st.progress(0, text="Entraînement en cours…")
        for i, name in enumerate(to_train):
            ...
            results[name] = {"model": m, "preds": preds,
                             "R²": round(r2, 4), "MAE": round(mae, 0),
                             "RMSLE": round(rmse, 4), "CV_mean": round(cv.mean(), 4),
    ...
    if st.session_state.metrics:
        ...
        perf_df = pd.DataFrame({
            k: {kk: vv for kk, vv in v.items() if kk not in ["model","preds"]}
        ...
        with vt2:
            ...
            if y_te_orig is not None:
                ...
                axes[1].set_xlabel("Erreur ($)"); axes[1].set_title("Résidus", fontweight="600")
                plt.tight_layout(); st.pyplot(fig, use_container_width=True); plt.close()
        ...
        with vt3:
            ...
            if not rf_models:
                st.info("Feature importance disponible pour Random Forest et Gradient Boosting.", icon="ℹ️")
            else:
                sel_fi = st.selectbox("Modèle", list(rf_models.keys()))
...
# ╚══════════════════════════════════════════════════════════════════╝
    ...
    if not st.session_state.trained_model:
        with st.expander("📦 Charger un modèle .pkl existant", expanded=True):
            uploaded_model = st.file_uploader("Charger model.pkl", type=["pkl","joblib"],
                                              help="Modèle sauvegardé avec joblib.")
            if uploaded_model:
                try:
                    ...
                    st.error(f"Erreur : {e}")
        if not st.session_state.trained_model:
            st.warning("⚠️ Entraînez d'abord un modèle en **Page 2**.", icon="⚠️")
            st.stop()
    ...
    with st.form("pred_form"):
        ...
        with r1c2: GrLivArea    = st.number_input("📐 Surface hab. (pi²)", 500, 6000, 1500, step=50)
        ...
        with r3c1: Neighborhood = st.selectbox("📍 Quartier", neigh_opts)
        with r3c2: MSZoning     = st.selectbox("🗺️ Zone", zone_opts)
        with r3c3: LotArea      = st.number_input("🌿 Terrain (pi²)", 1000, 100000, 8000, step=500)
    ...
    if submitted:
        ...
        input_dict = {
            ...
            "BldgType":"1Fam","HouseStyle":"2Story","OverallQual":int(OverallQual),
            ...
            "BsmtUnfSF":300,"TotalBsmtSF":int(TotalBsmtSF),"Heating":"GasA",
            "HeatingQC":"Ex","CentralAir":"Y","Electrical":"SBrkr",
            "1stFlrSF":int(GrLivArea)//2,"2ndFlrSF":int(GrLivArea)//2,
            "LowQualFinSF":0,"GrLivArea":int(GrLivArea),
        ...
        <div class="result-box">
          <div class="rlabel">Prix estimé — {st.session_state.model_name}</div>
          <div class="price">${price:,.0f}</div>
          <div class="rlabel">Fourchette indicative : ${price*0.90:,.0f} — ${price*1.10:,.0f}</div>
        </div>""", unsafe_allow_html=True)
        ...
        if df_ref is not None:
            ...
            metric_card(c3, "Écart médiane",   f"{abs(delta):.1f}%",
                        "au-dessus" if delta >= 0 else "en-dessous")
            ...
            ax.set_title("Position de votre bien dans le marché", fontweight="600")
...
# ╚══════════════════════════════════════════════════════════════════╝
    ...
    <div class="hero">
      ...
      <p>Supervision des accès et des événements de l'application.</p>
    </div>""", unsafe_allow_html=True)

    section("Statut de sécurité")
    s1, s2, s3, s4 = st.columns(4)
    metric_card(s1, "HTTPS",       "🔒 Actif",   "TLS Streamlit Cloud")
    ...
    checks = [
        ...
        ("🔐", "Authentification",         "Login requis avant toute utilisation. Credentials dans st.secrets."),
        ("🔑", "Secrets externalisés",     "Aucun mot de passe dans le code. Utilisation de st.secrets / Streamlit Cloud Secrets."),
        ("🔄", "Protection XSRF",          "enableXsrfProtection = true dans .streamlit/config.toml."),
        ("📋", "Logs d'activité",          "Chaque action est horodatée : connexion, upload, prédiction, export."),
        ("🚫", "Limite upload",            "Fichiers CSV limités à 10 Mo pour éviter les abus."),
        ("👁️", "Pas de tracking",         "gatherUsageStats = false dans config.toml."),
    ]
    ...
    if not logs:
        ...
        st.download_button(
            ...
            file_name=f"logs_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}.txt",
            mime="text/plain",
        )

    section("Configuration secrets Streamlit Cloud")
    info("<b>Settings → Secrets</b> dans votre app Streamlit Cloud → coller :")
    st.code("""
[auth]
username = "admin"
